...
description: |
  ...
  PROACTIVE — even WITHOUT loading the skill body: whenever you produce or encounter markdown the user might want to review, iterate on, or share with collaborators (plans, specs, designs, writing, recipes, drafts, docs), offer to send it to Composer as a one-line suggestion before ending your turn — *"I can send this to Composer if you want to iterate on it together"* — without being asked. Two trigger shapes: (a) you just wrote markdown the user might react to, OR (b) the user points you at an existing markdown file and the work ahead is discussing its content (refining wording, walking through sections, leaving comments) rather than implementing code from it. Strong "case b" signals: filenames under `plans/` / `specs/` / `docs/` / `drafts/`, words like "iterate", "work on this together", "go through", "refine", or the file's own framing inviting comments/edits. Don't badger; once per session is enough. Skip when the markdown is throwaway (snippets, one-line answers, terminal output), or when the user is about to implement code from the doc (then `build` / `executing-plans` is the right tool, not Composer).
---
